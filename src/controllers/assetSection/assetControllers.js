...
exports.createAsset = async (req, res) => {
  ...
  if (duplicateAssetID)
    return res
      .status(200)
      .json({ message: "Asset ID already exists", success: false });
  ...
  await newAsset
    ...
    .then(() => {
      res
        .status(200)
        .json({ message: "Asset has successfully added!", success: true });
    })
    .catch((err) => {
      res.status(500).json({
        message: "Asset has not inserted!",
        error: err.message,
        success: "false1",
      });
    });
...
exports.assignAsset = async (req, res) => {
  ...
  const asset = await Asset.findOne(
    { _id: ID },
    { _id: false, assetID: true, status: true }
  );
...
exports.assetsByCategory = async (req, res) => {
  ...
  const asset = await Asset.find({
    assetCategory: { $regex: new RegExp([CATEGORY?.toLowerCase()], "i") },
  })
    .then((assets) => {
    ...
    .catch((err) => {
      //console.log(err);
      res.status(500).send({
        message: "No assets like that category!",
        error: err.message,
        success: false,
      });
    });
...
exports.unassignAsset = async (req, res) => {
  ...
  if (asset.status == "Non-Available") {
    employeeList = await Asset_Lending.findOne(
  ...
  const docID = await Asset_Lending.findOne(
    { employeeID: empID, assetID: asset.assetID },
    { _id: true }
  ).sort({ _id: -1 });
  ...
  const unassignEmp = await Asset_Lending.updateMany(
    ...
    .catch((err) => {
      console.log(err);
      res.status(500).send({
        status: "Asset is not unassigned!",
        error: err.message,
        success: false,
      });
    });
...
exports.unavailableAssets = async (req, res) => {
  Asset.find({ status: "Non-Available" })
    .then((assets) => {
      res.json(assets);
    })
    .catch((err) => {
...
exports.assignPerson = async (req, res) => {
  ...
  if (asset.status == "Non-Available") {
    const employeeList = await Asset_Lending.findOne(
      { assetID: asset.assetID },
      { employeeID: 1 }
    ).sort({ _id: -1 });
...
exports.isAssigned = async (req, res) => {
  ...
  if (empID) {
    const lenderAsset = await Asset_Lending.findOne(
      { employeeID: empID, reassignDate: null },
      { employeeID: 1, assetID: 1 }
    ).sort({ _id: -1 });
    if (lenderAsset) {
      res.status(200).send({
        assetID: lenderAsset.assetID,
        message:
          "You cannot change the status to resign without give the lendered asset back to the company",
        success: true,
      });
    } else {
      const updateEmpStatus = await Employee.updateOne(
        ...
        .then(() => {
          res
            .status(200)
            .send({
              message: "Changed the employee status to the resign!",
              success: "true1",
            });
        })
        .catch((err) => {
